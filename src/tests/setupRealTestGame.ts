...
export async function setupTestGame() {
	...
	await Turn.bulkCreate([
		{ playerOrder: 0, gameId: 1, userId: '220525113404030987' },
		{ playerOrder: 1, gameId: 1, userId: '540270864143220805' },
	]);
	const properties = getProperties();

	properties.forEach(({ id }: { id: number }) => {
		PropertyGame.create({ id: id, gameId: 1, ownerId: '220525113404030987' });
	});
}
