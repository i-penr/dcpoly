...
export async function propertyTurn(
	...
	if (!property || !propertyGame || !responseBuilder.embeds[0])
		throw new Error('Property does not exist (internal error).');
	...
	if (!owner) {
		responseBuilder.embeds[0].setDescription(
			`This property is not owned by anyone.\n\nWhat do you want to do?\n\n- **Current Money** \`${player.money}$\`\n- **Price** \`${property.price}\``,
		);
		responseBuilder.actionRow.addComponents(
			createPropertyPromptActionRow(player.get('money') >= property.price),
		);
		return;
	}
