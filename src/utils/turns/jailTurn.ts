...
export async function promptJailActionAndCheckIfPlays(
	player: Player,
	...
	const jailEmbed = buildTemplateEmbed()
		...
		.setDescription(
			`You have \`${player.jailStatus}\` turns remaining in jail
	...
	const buttons = [
		{ id: 'rollDices', label: 'Roll dices (Doubles = out of jail)', style: ButtonStyle.Primary },
		{
			id: 'payUp',
...
async function waitForJailResponse(
