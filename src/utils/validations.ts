...
export async function getPlayerTurn(game: Game, player: Player): Promise<Turn> {
	const playerTurn = await Turn.findOne({
		where: { gameId: game.get('id'), userId: player.get('userId') },
	});
...
export async function validateTurn(game: Game, playerTurn: Turn): Promise<void> {
	...
	if (await playerHasRolled(playerTurn))
		throw new Error('You have already rolled. Finish your turn by clicking the `End Turn` button');
}
