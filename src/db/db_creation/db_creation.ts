...
export function setupDatabaseAssociations() {
	...
	// Player-Turn 1:1
	Player.hasOne(Turn, { foreignKey: 'userId' });
	Turn.belongsTo(Player, { foreignKey: 'userId' });

	// Property-Player 1:N
	Player.hasMany(PropertyGame, { foreignKey: 'ownerId', sourceKey: 'userId' });
	PropertyGame.belongsTo(Player, { foreignKey: 'ownerId', as: 'owner', targetKey: 'userId' });

	// Property-Game 1:N
	Game.hasMany(PropertyGame, { foreignKey: 'gameId' });
	PropertyGame.belongsTo(Game, { foreignKey: 'gameId' });
}
