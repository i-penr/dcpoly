...
const command: Command = {
	data: new SlashCommandBuilder().setName('newgame').setDescription('Create a new game.'),
	async execute(interaction: ChatInputCommandInteraction) {
		try {
			...
			const newGame = await Game.create({ guild_id: interaction.guildId! });

			interaction.reply(`New game #${newGame.get('id')} created sucessfully.`);
		} catch (error) {
			console.log(error);
			interaction.reply(
				buildErrorEmbed(interaction, 'Something went wrong when creating a new game.'),
			);
		}
	},
};
